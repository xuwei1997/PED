...
if __name__ == "__main__":
    ...
    # ----------------------------------------------------------------------------------------------------------------------------#
    model_path = "/home/dell/python/Cherry_unsup/self_supervised_resnet/train_voc07_resnet_encoder.h5"
    # model_path = "/tmp/pycharm_project_116/self_supervised/self_logs/ep001-loss0.556-val_loss0.482.h5"
    # model_path = '/tmp/pycharm_project_116/self_supervised2/self_logs/h5_2022_03_21_12_18_53/ep004-loss1.062-val_loss1.041.h5'
    # model_path = '/home/dell/python/Cherry_unsup/model_data/unet_resnet_voc.h5'
    # model_path0 ='/home/dell/python/Cherry_unsup/model_data/unet_resnet_voc.h5'
    # -------------------------------#
    #   输入图片的大小
    # h w
    # -------------------------------#
    ...
    # ----------------------------------------------------#
    UnFreeze_Epoch = 120 # 最终轮数 80
    # UnFreeze_Epoch = 2  # 最终轮数
    Unfreeze_batch_size = 8  # 已修改
    Unfreeze_lr = 1e-5
    # Unfreeze_lr = 2e-6
    # Unfreeze_lr = 1e-4
    # -------------------------------#
    ...
    # cls_weights = np.array([5,4,1,5,2], np.float32)
    # ---------------------------------------------------------------------#
    ...
    if model_path != '':
        ...
        #   载入预训练权重
        # ------------------------------------------------------#
        # model.load_weights(model_path0, by_name=True, skip_mismatch=True)
    ...
    os.mkdir('logs/' + time_str)
    checkpoint = ModelCheckpoint('logs/' + time_str + 'ep{epoch:03d}-loss{loss:.3f}-val_loss{val_loss:.3f}.h5',
                                 monitor='val_loss', save_weights_only=True, save_best_only=False, period=1)
    ...
    if True:
        ...
        model.fit_generator(
            ...
            use_multiprocessing=True if num_workers > 1 else False,
            workers=num_workers,
            callbacks=[logging, checkpoint, reduce_lr, loss_history, early_stopping]
        )
    ...
    score=model.evaluate_generator(generator=test_dataloader,steps=epoch_step_test)
    print(score)
